...
- Store account data and encrypted API credentials on the server after sign-in, with per-account isolation.
- Synchronize canvas, assets, generation history, configuration, prompt sources, plugins, and theme preferences across devices with conflict retries and deletion tombstones.
- Use WebDAV for optional cross-device synchronization.

## Current limitations

- WebDAV and local Agent credentials remain device-local; provider requests are still sent directly from the browser.
