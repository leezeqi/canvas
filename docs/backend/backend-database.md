...
视频生成任务表。后端创建视频任务后写入该表，后台轮询器每 6 秒统一查询未完成任务并更新进度、完成地址或失败详情；前端刷新、切换页面或关闭浏览器不会影响后端继续轮询。
...
| `next_poll_at` | string | 遇到限流或瞬时错误后的下次允许轮询时间 |
| `poll404_count` | number | Sub2API 连续 404 次数，成功响应后清零 |
| `poll_retry_count` | number | Sub2API 连续 429 或 5xx 重试次数，成功响应后清零 |
