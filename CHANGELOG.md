...
+ [修复] 模型选择弹窗在后台刷新时保留拉取结果、勾选和输入，关闭或切换渠道后忽略旧请求结果。
+ [修复] 渠道编辑草稿不再被后台同步重置，配置同步保留请求期间的新保存并自动补传，避免切回网页后地址和 API Key 还原。
+ [新增] 点击在线人数可查看脱敏用户列表，优先显示昵称、无昵称时显示邮箱，服务端仅向已登录用户返回脱敏信息。
+ [修复] Windows 下拒绝以 `/` 开头的同步文件路径，避免 Unix 风格绝对路径绕过校验。
+ [新增] 顶部导航显示最近五分钟内活跃的已登录账号人数，同一账号多标签页合并计数。
+ [修复] 账号登录不再等待全量媒体同步，后台同步增加单请求与整轮超时，失败时保留本地数据并支持重试。
+ [优化] 同一账号的跨标签页同步使用浏览器锁，减少并发写入导致的同步冲突。
+ [调整] 隐藏前端顶部说明文档入口，并将原 GitHub 按钮改为跳转 Hajimi。
+ [修复] 配置同步冲突按本地快照识别实际改动，避免旧浏览器在重新联网时覆盖服务器渠道地址。
+ [修复] 账号登录同步配置时以服务器已有渠道配置为准，避免新浏览器或旧缓存覆盖另一设备的渠道地址。
+ [修复] 生成记录新增、删除和状态更新时主动触发账号同步，确保其他设备及时收到生图和视频记录变更。
+ [修复] 新浏览器登录时先按远端生成记录恢复媒体文件，避免历史图片因旧 Blob URL 失效而显示为空。
+ [修复] 远端已有生成记录但媒体清单不完整时，使用原设备本地文件自动补传，恢复被中断的首次同步。
+ [修复] 将同步接口的 Nginx 上传上限与后端 25 MiB 媒体文件限制对齐，避免正常媒体上传被默认 1 MiB 限制拦截。
+ [新增] 建立按账号隔离的服务端数据体系，支持画布、素材、生成记录、配置、提示词源、插件和主题跨设备同步，并为媒体文件提供持久化存储。
+ [优化] 同步接口增加乐观并发冲突重试、删除墓碑、媒体文件权限校验和 API Key 字段级加密。
+ [新增] 增加独立邮箱登录、会话保护和 Hajimi 一次性票据登录入口，两边账号会话与业务数据保持隔离。
...
+ [修复] Gemini 生图兼容以 Markdown 图片链接返回的图片响应。
