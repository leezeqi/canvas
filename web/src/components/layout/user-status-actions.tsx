...
export function UserStatusActions({ className, compact = false, showConfig = true, variant = "default", onOpenShortcuts, accountOpen, onAccountOpenChange, accountRef, getPopupContainer }: UserStatusActionsProps) {
    ...
    const naturalIconClass = "inline-flex size-7 shrink-0 items-center justify-center rounded-lg text-foreground/60 transition hover:bg-foreground/8 hover:text-foreground [&_svg]:size-4";
    ...
    return (
        <div className={cn("inline-flex shrink-0 items-center gap-1", className)}>
            ...
            {!user ? (
                <Link href="/login" className="px-1.5 text-sm font-medium text-slate-600 underline-offset-4 transition hover:text-foreground hover:underline" style={iconStyle}>
                    登录
                </Link>
            ) : null}
            {user ? (
                    ...
                    <Dropdown open={accountOpen} onOpenChange={onAccountOpenChange} trigger={["click"]} placement="bottomRight" getPopupContainer={getPopupContainer} styles={{ root: { minWidth: 150 } }} menu={{ items: menuItems }}>
                        <button type="button" className="flex size-7 shrink-0 items-center justify-center rounded-full bg-transparent p-0 text-[0] leading-[0] transition" aria-label="账户菜单">
                            <Avatar
                                ...
                                alt={userName}
                                className="!flex !items-center !justify-center border border-slate-300 bg-transparent text-[11px] font-semibold text-slate-800 transition hover:border-foreground hover:text-foreground dark:border-slate-700 dark:text-slate-100 dark:hover:border-white dark:hover:text-white"
                                style={avatarStyle}
                            >
                                {avatarText}
                            </Avatar>
